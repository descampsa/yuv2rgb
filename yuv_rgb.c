...
for ITU-R BT.601-7 values are derived from equations in sections 2.5.1-2.5.3, 
assuming RGB is encoded using full range ([0-1]<->[0-255])

for ITU-R BT.709-6 values are derived from equations in sections 3.2-3.4, 
assuming RGB is encoded using full range ([0-1]<->[0-255])
...
static const YUV2RGBParam YUV2RGB[3] = {
    // ITU-T T.871 (JPEG)
    {
        .y_shift=0, .y_factor=V(1.0), .v_r_factor=V(1.402), 
        .u_g_factor=-V(0.3441), .v_g_factor=-V(0.7141), 
        .u_b_factor=V(1.772)
    },
    // ITU-R BT.601-7
    {
        .y_shift=16, .y_factor=V(1.1644), .v_r_factor=V(1.596),
        .u_g_factor=-V(0.3918), .v_g_factor=-V(0.813),
        .u_b_factor=V(2.0172)
    },
    // ITU-R BT.709-6
    {
        .y_shift=16, .y_factor=V(1.1644), .v_r_factor=V(1.7927),
        .u_g_factor=-V(0.2132), .v_g_factor=-V(0.5329),
        .u_b_factor=V(2.1124)
    }
};
...
static const RGB2YUVParam RGB2YUV[3] = {
    // ITU-T T.871 (JPEG)
    {
        .y_shift=0,
        .matrix={
            {V(0.299), V(0.587), V(0.114)},
            {-V(0.1687), -V(0.3313), V(0.5)},
            {V(0.5), -V(0.4187), -V(0.0813)}
        }
    },
    // ITU-R BT.601-7
    {
        .y_shift=16,
        .matrix={
            {V(0.2568), V(0.5041), V(0.0979)},
            {-V(0.1482), -V(0.291), V(0.4392)},
            {V(0.4392), -V(0.3678), -V(0.0714)}
        }
    },
    // ITU-R BT.709-6
    {
        .y_shift=16,
        .matrix={
            {V(0.1826), V(0.6142), V(0.062)},
            {-V(0.1006), -V(0.3386), V(0.4392)},
            {V(0.4392), -V(0.3989), -V(0.0403)}
        }
    }
};
...
static const uint8_t LUT[512] = {
    ...
    240,241,242,243,244,245,246,247,248,249,250,251,252,253,254,255,255,255,255,255,
...
void yuv420_rgb24_std(uint32_t width, uint32_t height, const uint8_t *Y, 
...
void rgb24_yuv420_std(uint32_t width, uint32_t height, const uint8_t *RGB,
                      uint32_t RGB_stride, uint8_t *Y, uint8_t *U,
                      uint8_t *V, uint32_t Y_stride, uint32_t UV_stride, 
                      YCbCrType yuv_type) {
...
void yuv420_rgb24_sse(uint32_t width, uint32_t height, const uint8_t *Y,
                      const uint8_t *U, const uint8_t *V, uint32_t Y_stride,
                      uint32_t UV_stride, uint8_t *RGB, uint32_t RGB_stride, 
                      YCbCrType yuv_type) {
...
void yuv420_rgb24_sseu(uint32_t width, uint32_t height, const uint8_t *Y, 
                       const uint8_t *U, const uint8_t *V, uint32_t Y_stride, 
                       uint32_t UV_stride, uint8_t *RGB, uint32_t RGB_stride, 
                       YCbCrType yuv_type) {
